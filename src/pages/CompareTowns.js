...
const useStyles = makeStyles(theme => ({
  ...
  const ComapreTowns = () => {
    ...
    const getAllTownsData = () => {
        return {
            labels:_.range(2,16).filter((n, i) => showTier[i]).map(i => i % 2 === 0 ? (i/2).toString() : Math.floor(i/2).toString().concat('+')),
            datasets: _.values(_.mapValues(creatureData, townData => {
                const townName = townData[0].Town
                return {
                    ...defaultLineOptions,
                    borderColor: townColors[townName],
                    pointHoverBackgroundColor: townColors[townName],
                    pointHoverBorderColor: townColors[townName],
                    label: townData[0].Town,
                    fill: false,
                    data: townData.filter((n, i) => showTier[i]).map(c => normalizeWithGold ? c[currentStat] / c.Cost : c[currentStat])
                }
            }))
